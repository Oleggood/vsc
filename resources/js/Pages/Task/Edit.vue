        ...
        <div class="flex justify-center mt-4">
            <form class="mt-4 w-full" @submit.prevent="update(task)">
                ...
                <div class="">
                    ...
                    />
                    <InputLabel class="mt-2" for="new_task_done" value="Отметки об исполнении:"/>
                    <MyTextArea
                        id="new_task_done"
                        class="block w-3/4"
                        v-model="formLocalCreate.done"
                    />
...
export default {
    ...
    mounted() {
        this.usersOnly = this.usersTask.map(function (value, index) {
            return value['users'];
        });
        this.formLocalCreate.users = this.usersOnly[0];
    },
    ...
    methods: {
        ...
        update(task) {
            ...
            if (this.formLocalCreate.users.length >= this.usersOnly.length) {
                this.formLocalCreate.task_status_id = 1; // Установка статуса "Исполнена"
            } else if (this.formLocalCreate.users.length > 0) {
                this.formLocalCreate.task_status_id = 2; // Установка статуса "Частично исполнена"
            } else {
                this.formLocalCreate.task_status_id = 3; // Установка статуса "Не исполнена"
            }


            this.form = this.$inertia.form(this.formLocalCreate);
            console.log(this.form);
            this.form.patch(`/task/${task.id}`, {
                onFinish: () => {
