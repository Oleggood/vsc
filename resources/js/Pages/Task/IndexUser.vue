...
<template>
    <PublicMain>
        <div class="flex justify-center">
            <h3 class="text-lg font-semibold"> Задачи для {{ listTitle() }} </h3>
        </div>
        <div class="inline-block">
        <div v-show="userIsManager" class="mt-4">
            <label class="flex items-center">
                <Checkbox name="is_director" v-model:checked="this.selPage" value=""/>
                <span class="ml-2 text-sm text-gray-600">показать все задачи моего подразделения</span>
            </label>
        </div>
        </div>

        <!--index-->
        <!--блок с задачами пользователя - начало-->
        <div v-show="!this.selPage">
            <div v-if="tasksUser.length > 0" class="mt-6">
                <table class="border-collapse border border-slate-500 w-full">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'task_date')">
                                Дата задачи
                            </div>
                            <div class="relative">
                                <div class="absolute" v-show="sortParam === 'task_date'">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'number')">
                                Номер документа
                            </div>
                            <div class="relative">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'task_name')">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'item')">
                                Пункт
                            </div>
                            <div class="relative">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'task')">
                                Текст поручения
                            </div>
                            <div class="relative">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'note')">
                                Примечание
                            </div>
                            <div class="relative">
                    ...
                    <th class="border border-slate-600 py-2 pr-2 text-sm">
                        <div class="flex justify-center">
                            <div class="cursor-pointer" title="сортировка"
                                 @click="sortTextItemsUser(sortParam = 'deadline')">
                                Дата исполнения
                            </div>
                            <div class="relative">
                                <div class="absolute" v-show="sortParam === 'deadline'">
                                    <svg version="1.1" id="Layer_1"
                                         xmlns="http://www.w3.org/2000/svg"
                                         xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
                                         y="0px" width="8px"
                    ...
                    <tbody>
                    <tr v-for="(task, index) in tasksUser" :key="task.id" @dblclick="textMarker">
                        <td class="border border-slate-700 text-center">
                            <Link :href="route('taskuser.show', task.id)">
                        ...
                        <td class="border border-slate-700 text-sm italic">
                            <div class="flex justify-between">
                                ...
                                <div title="Имеются комментарии" class="px-0.5">
            ...
            <!--count-->
            <div class="mt-4">
                <p>Количество задач сотрудника: {{ tasksUser.length }}</p>
            </div>
            <!--/count-->
            <!--блок с задачами пользователя - конец-->
        </div>


        <!--блок с задачами подразделения - начало-->
        <div v-show="this.selPage">
            ...
            <h3 v-else class="mt-6 text-gray-900">Здесь пока пусто...</h3>
            <!--/index 2-->
            <!--count-->
            <div class="mt-4">
                <p>Количество задач подразделения: {{ tasks.length }}</p>
            </div>
            <!--/count-->
        </div>
        <!--блок с задачами подразделения - конец-->
...
export default {
    ...
    methods: {
        ...
        sortTextItemsUser(sortParam) {
            this.tasksUser.sort((item1, item2) => {
        ...
        listTitle() {
            this.selPage ? (this.title = `${this.divisionName}`) : (this.title = `${this.userName} ${this.userSurName}`);
            return this.title;
        },
